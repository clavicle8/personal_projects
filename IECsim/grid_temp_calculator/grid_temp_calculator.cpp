...
double T_amb = 293.0; // room temp in K
...
void calculate(double I, double d_wire) {
    double r_wire = d_wire/2;  //d-wire in cm
    double csa = pi * pow(r_wire, 2);
    double l = (pi * r_ring) / 2;
    double A_m2 = 24 * pi * r_wire * l / 1e4;

    double R_segment = (rho_r * l) / csa;
    ...
    */
    ...
    double T_eq = pow(P_in / (sigma * epsilon * A_m2) + pow(T_amb, 4), 0.25);

    printf("I = %.3f A, r_wire = %.4f cm\n", I, r_wire);
    printf("R_segment = %f ohm\n", R_segment);
    printf("R_eff = %f ohm\n", R_eff);
    printf("P_in = %f W\n", P_in);
    printf("A = %e m^2\n", A_m2);
    printf("T_eq = %.1f K\n", T_eq);
    printf("Melting point = %.1f K\n", melting_point);
    printf("Will melt: %s\n\n", T_eq >= melting_point ? "yes" : "no");
